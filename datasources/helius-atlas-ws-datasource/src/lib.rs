...
static ACCOUNT_DELETIONS_RECEIVED: Counter = Counter::new(
    ...
    "Total account deletions received from Helius Atlas WebSocket",
);
...
static ACCOUNT_UPDATES_RECEIVED: Counter = Counter::new(
    ...
    "Total account updates received from Helius Atlas WebSocket",
);
...
static TRANSACTION_UPDATES_RECEIVED: Counter = Counter::new(
    ...
    "Total transaction updates received from Helius Atlas WebSocket",
);
...
fn init_histograms() {
    CLOCK_PROCESS_TIME_NANOS.get_or_init(|| {
        Histogram::new(
            ...
            "Time taken to process clock updates in nanoseconds",
            vec![
                1_000.0,
                10_000.0,
                100_000.0,
    ...
    ACCOUNT_DELETION_PROCESS_TIME_NANOS.get_or_init(|| {
        Histogram::new(
            ...
            "Time taken to process account deletion updates in nanoseconds",
            vec![
                1_000.0,
                10_000.0,
                100_000.0,
                                    ...
                                    Some(clock_event) => {
                                        ...
                                        if let Some(clock_data) = clock_event.value.decode::<Account>() {
                                            if let Ok(clock) = bincode::deserialize::<Clock>(&clock_data.data) {
                                                ...
                                                CLOCK_PROCESS_TIME_NANOS
                                                    .get()
                                                    .unwrap()
                                                    .record(last_clock_update.elapsed().as_nanos() as f64);
                                            }
                                        }
                                    }
                                    None => {
                                            ...
                                            Some(acc_event) => {
                                                ...
                                                if decoded_account.lamports == 0 && decoded_account.data.is_empty() && decoded_account.owner == solana_system_interface::program::ID {
                                                    ...
                                                    if !accounts_tracked.is_empty() && accounts_tracked.contains(&account) {
                                                        ...
                                                        ACCOUNT_DELETION_PROCESS_TIME_NANOS
                                                            .get()
                                                            .unwrap()
