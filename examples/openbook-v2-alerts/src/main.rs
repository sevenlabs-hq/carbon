        ...
        async move {
            ...
            match instruction.data.clone() {
                ...
                OpenbookV2Instruction::CloseMarket(close_market) => {
                    log::info!(
                        "CloseMarket: signature: {signature}, close_market: {close_market:?}"
                    );
                }
                ...
                OpenbookV2Instruction::PlaceOrders(place_orders) => {
                    log::info!(
                        "PlaceOrders: signature: {signature}, place_orders: {place_orders:?}"
                    );
                }
                ...
                OpenbookV2Instruction::CancelOrder(cancel_order) => {
                    log::info!(
                        "CancelOrder: signature: {signature}, cancel_order: {cancel_order:?}"
                    );
                }
                OpenbookV2Instruction::CancelOrderByClientOrderId(
                ...
                OpenbookV2Instruction::SettleFunds(settle_funds) => {
                    log::info!(
                        "SettleFunds: signature: {signature}, settle_funds: {settle_funds:?}"
                    );
                }
                ...
                OpenbookV2Instruction::SetDelegate(set_delegate) => {
                    log::info!(
                        "SetDelegate: signature: {signature}, set_delegate: {set_delegate:?}"
                    );
                }
                ...
                OpenbookV2Instruction::PruneOrders(prune_orders) => {
                    log::info!(
                        "PruneOrders: signature: {signature}, prune_orders: {prune_orders:?}"
                    );
                }
                ...
                OpenbookV2Instruction::OpenOrdersPositionLogEvent(
                    open_orders_position_log_event,
                ) => {
                    log::info!(
