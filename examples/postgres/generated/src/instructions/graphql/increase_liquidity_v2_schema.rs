use juniper::GraphQLObject;
use carbon_core::graphql::primitives::U128;
use carbon_core::graphql::primitives::U64;

#[derive(Debug, Clone, GraphQLObject)]
#[graphql(name = "IncreaseLiquidityV2")]
pub struct IncreaseLiquidityV2GraphQL {
        pub liquidity: U128,
        pub amount0_max: U64,
        pub amount1_max: U64,
        pub base_flag: Option<bool>,
}

impl From<crate::instructions::postgres::IncreaseLiquidityV2Row> for IncreaseLiquidityV2GraphQL {
    fn from(row: crate::instructions::postgres::IncreaseLiquidityV2Row) -> Self {
        Self {
            liquidity: carbon_core::graphql::primitives::U128(*row.liquidity),
            amount0_max: carbon_core::graphql::primitives::U64(*row.amount0_max),
            amount1_max: carbon_core::graphql::primitives::U64(*row.amount1_max),
            base_flag: row.base_flag.map(|v| v),
        }
    }
}