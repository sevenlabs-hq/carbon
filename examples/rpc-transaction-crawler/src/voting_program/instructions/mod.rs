...
use crate::VotingProgramDecoder;
pub mod cast_vote;
pub mod create_vote;
...
impl InstructionDecoder for VotingProgramDecoder {
    ...
    fn decode_instruction(
        ...
        if let Some(decoded_instruction) = cast_vote::CastVote::deserialize(instruction.data.as_slice()) {
            return Some(carbon_core::instruction::DecodedInstruction {
                program_id: instruction.program_id,
                data: VotingProgramInstruction::CastVote(decoded_instruction),
            });
        }
        if let Some(decoded_instruction) = create_vote::CreateVote::deserialize(instruction.data.as_slice()) {
            return Some(carbon_core::instruction::DecodedInstruction {
                program_id: instruction.program_id,
                data: VotingProgramInstruction::CreateVote(decoded_instruction),
            });
        }
        if let Some(decoded_instruction) = cast_vote_event::CastVoteEvent::deserialize(instruction.data.as_slice()) {
            return Some(carbon_core::instruction::DecodedInstruction {
                program_id: instruction.program_id,
                data: VotingProgramInstruction::CastVoteEvent(decoded_instruction),
            });
        }
        if let Some(decoded_instruction) = create_vote_event::CreateVoteEvent::deserialize(instruction.data.as_slice()) {
            return Some(carbon_core::instruction::DecodedInstruction {
                program_id: instruction.program_id,
