...
/// - `trade_enable_timestamp` - The timestamp when trading is enabled for this
///   pool (within 72 hours)
///
/// #### Special Errors
/// `InvalidTokenMintOrder` - The order of mints have to be ordered by
/// `SqrtPriceOutOfBounds` - provided initial_sqrt_price is not between 2^-64 to
/// 2^64 `InvalidTradeEnableTimestamp` - provided trade_enable_timestamp is not
/// within 72 hours or the adaptive fee-tier is permission-less
/// `UnsupportedTokenMint` - The provided token mint is not supported by the
/// program (e.g. it has risky token extensions)
...
impl CarbonDeserialize for InitializePoolWithAdaptiveFee {
    const DISCRIMINATOR: &'static [u8] = &[143, 94, 96, 76, 172, 124, 119, 199];
...
impl ArrangeAccounts for InitializePoolWithAdaptiveFee {
