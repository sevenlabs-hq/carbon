...
impl carbon_core::clickhouse::ClusterTable for UpdateActivationPointRow {
    fn local_table() -> &'static str {
        "meteora_pools_update_activation_point_instruction_local"
    }
...
impl carbon_core::clickhouse::Table for UpdateActivationPointRow {
    fn table() -> &'static str {
        "meteora_pools_update_activation_point_instruction"
    }
}
...
pub struct UpdateActivationPointRowMigrationOperation;

#[cfg(feature = "clickhouse-cluster")]
#[async_trait::async_trait]
impl carbon_core::clickhouse::Operation for UpdateActivationPointRowMigrationOperation {
    async fn up(
        &self,
        client: &clickhouse::Client,
        ...
        let replica_path = format!(
            "/clickhouse/tables/{}/meteora_pools_update_activation_point_instruction_local",
            config.shard_name,
        );
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS meteora_pools_update_activation_point_instruction_local ON CLUSTER {cluster} (
                        __signature String,
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS meteora_pools_update_activation_point_instruction ON CLUSTER {cluster} AS meteora_pools_update_activation_point_instruction_local
                    ENGINE = Distributed({cluster}, currentDatabase(), meteora_pools_update_activation_point_instruction_local, rand())
                "#,
                cluster = cluster_name,
            ))
            .execute()
    ...
    async fn down(
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS meteora_pools_update_activation_point_instruction ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS meteora_pools_update_activation_point_instruction_local ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
...
impl carbon_core::clickhouse::Operation for UpdateActivationPointRowMigrationOperation {
    async fn up(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query(
                r#"
                    CREATE TABLE IF NOT EXISTS meteora_pools_update_activation_point_instruction (
                        __signature String,
                        __instruction_index UInt32,
                        __stack_height UInt32,
                        __slot UInt64,
    ...
    async fn down(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query("DROP TABLE IF EXISTS meteora_pools_update_activation_point_instruction")
            .execute()
            .await?;
