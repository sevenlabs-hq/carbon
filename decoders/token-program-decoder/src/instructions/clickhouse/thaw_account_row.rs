...
impl carbon_core::clickhouse::ClusterTable for ThawAccountRow {
    fn local_table() -> &'static str {
        "token_program_thaw_account_instruction_local"
    }

    fn distributed_table() -> &'static str {
        "token_program_thaw_account_instruction"
    }
}
...
impl carbon_core::clickhouse::Operation for ThawAccountRowMigrationOperation {
    async fn up(
        ...
        let replica_path = format!(
            "/clickhouse/tables/{}/token_program_thaw_account_instruction_local",
            config.shard_name,
        );
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS token_program_thaw_account_instruction_local ON CLUSTER {cluster} (
                        __signature String,
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS token_program_thaw_account_instruction ON CLUSTER {cluster} AS token_program_thaw_account_instruction_local
                    ENGINE = Distributed({cluster}, currentDatabase(), token_program_thaw_account_instruction_local, rand())
                "#,
                cluster = cluster_name,
            ))
            .execute()
    ...
    async fn down(
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS token_program_thaw_account_instruction ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS token_program_thaw_account_instruction_local ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
...
impl carbon_core::clickhouse::Operation for ThawAccountRowMigrationOperation {
    async fn up(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query(
                r#"
                    CREATE TABLE IF NOT EXISTS token_program_thaw_account_instruction (
                        __signature String,
                        __instruction_index UInt32,
                        __stack_height UInt32,
                        __slot UInt64,
    ...
    async fn down(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query("DROP TABLE IF EXISTS token_program_thaw_account_instruction")
            .execute()
            .await?;
