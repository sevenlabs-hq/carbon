...
impl carbon_core::clickhouse::ClusterTable for UiAmountToAmountRow {
    fn local_table() -> &'static str {
        "token_program_ui_amount_to_amount_instruction_local"
    }

    fn distributed_table() -> &'static str {
        "token_program_ui_amount_to_amount_instruction"
    }
}
...
impl carbon_core::clickhouse::Operation for UiAmountToAmountRowMigrationOperation {
    async fn up(
        ...
        let replica_path = format!(
            "/clickhouse/tables/{}/token_program_ui_amount_to_amount_instruction_local",
            config.shard_name,
        );
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS token_program_ui_amount_to_amount_instruction_local ON CLUSTER {cluster} (
                        __signature String,
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS token_program_ui_amount_to_amount_instruction ON CLUSTER {cluster} AS token_program_ui_amount_to_amount_instruction_local
                    ENGINE = Distributed({cluster}, currentDatabase(), token_program_ui_amount_to_amount_instruction_local, rand())
                "#,
                cluster = cluster_name,
            ))
            .execute()
    ...
    async fn down(
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS token_program_ui_amount_to_amount_instruction ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS token_program_ui_amount_to_amount_instruction_local ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
...
impl carbon_core::clickhouse::Operation for UiAmountToAmountRowMigrationOperation {
    async fn up(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query(
                r#"
                    CREATE TABLE IF NOT EXISTS token_program_ui_amount_to_amount_instruction (
                        __signature String,
                        __instruction_index UInt32,
                        __stack_height UInt32,
                        __slot UInt64,
    ...
    async fn down(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query("DROP TABLE IF EXISTS token_program_ui_amount_to_amount_instruction")
            .execute()
            .await?;
