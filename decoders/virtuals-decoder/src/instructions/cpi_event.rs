...
pub enum CpiEvent {
    ...
    GraduationEvent(events::graduation_event::GraduationEventEvent),
    LaunchEvent(events::launch_event::LaunchEventEvent),
...
impl CarbonDeserialize for CpiEvent {
    ...
    fn decode(data: &[u8]) -> Option<Self> {
        ...
        if let Some(decoded) = events::graduation_event::GraduationEventEvent::decode(event_data) {
            return Some(CpiEvent::GraduationEvent(decoded));
        }
        if let Some(decoded) = events::launch_event::LaunchEventEvent::decode(event_data) {
            return Some(CpiEvent::LaunchEvent(decoded));
        }
