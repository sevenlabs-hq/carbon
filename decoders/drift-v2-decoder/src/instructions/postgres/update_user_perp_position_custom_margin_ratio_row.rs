//! This code was AUTOGENERATED using the Codama library.
use carbon_core::{
    instruction::InstructionMetadata,
    postgres::{metadata::InstructionRowMetadata, primitives::U16},
};

#[derive(sqlx::FromRow, Debug, Clone)]
pub struct UpdateUserPerpPositionCustomMarginRatioRow {
    #[sqlx(flatten)]
    pub instruction_metadata: InstructionRowMetadata,
    pub sub_account_id: U16,
    pub perp_market_index: U16,
    pub margin_ratio: U16,
    #[sqlx(rename = "__accounts")]
    pub accounts: sqlx::types::Json<Vec<solana_instruction::AccountMeta>>,
}

impl UpdateUserPerpPositionCustomMarginRatioRow {
    pub fn from_parts(
        source: crate::instructions::update_user_perp_position_custom_margin_ratio::UpdateUserPerpPositionCustomMarginRatio,
        metadata: InstructionMetadata,
        accounts: Vec<solana_instruction::AccountMeta>,
    ) -> Self {
        Self {
            instruction_metadata: metadata.into(),
            sub_account_id: source.sub_account_id.into(),
            perp_market_index: source.perp_market_index.into(),
            margin_ratio: source.margin_ratio.into(),
            accounts: sqlx::types::Json(accounts),
        }
    }
}

impl TryFrom<UpdateUserPerpPositionCustomMarginRatioRow> for crate::instructions::update_user_perp_position_custom_margin_ratio::UpdateUserPerpPositionCustomMarginRatio {
    type Error = carbon_core::error::Error;
    fn try_from(source: UpdateUserPerpPositionCustomMarginRatioRow) -> Result<Self, Self::Error> {
        Ok(Self {
            sub_account_id: source.sub_account_id.try_into().map_err(|_| carbon_core::error::Error::Custom("Failed to convert value from postgres primitive".to_string()))?,
            perp_market_index: source.perp_market_index.try_into().map_err(|_| carbon_core::error::Error::Custom("Failed to convert value from postgres primitive".to_string()))?,
            margin_ratio: source.margin_ratio.try_into().map_err(|_| carbon_core::error::Error::Custom("Failed to convert value from postgres primitive".to_string()))?,
        })
    }
}

impl carbon_core::postgres::operations::Table for crate::instructions::update_user_perp_position_custom_margin_ratio::UpdateUserPerpPositionCustomMarginRatio {
    fn table() -> &'static str {
        "update_user_perp_position_custom_margin_ratio_instruction"
    }

    fn columns() -> Vec<&'static str> {
        vec![
            "__signature",
            "__instruction_index",
            "__stack_height",
            "__slot",
            "sub_account_id",
            "perp_market_index",
            "margin_ratio",
            "__accounts",
        ]
    }
}

#[async_trait::async_trait]
impl carbon_core::postgres::operations::Insert for UpdateUserPerpPositionCustomMarginRatioRow {
    async fn insert(&self, pool: &sqlx::PgPool) -> carbon_core::error::CarbonResult<()> {
        sqlx::query(
            r#"
            INSERT INTO update_user_perp_position_custom_margin_ratio_instruction (
                "sub_account_id",
                "perp_market_index",
                "margin_ratio",
                __signature, __instruction_index, __stack_height, __slot, __accounts
            ) VALUES (
                $1, $2, $3, $4, $5, $6, $7, $8
            )"#,
        )
        .bind(self.sub_account_id)
        .bind(self.perp_market_index)
        .bind(self.margin_ratio)
        .bind(&self.instruction_metadata.signature)
        .bind(self.instruction_metadata.instruction_index)
        .bind(self.instruction_metadata.stack_height)
        .bind(&self.instruction_metadata.slot)
        .bind(&self.accounts)
        .execute(pool)
        .await
        .map_err(|e| carbon_core::error::Error::Custom(e.to_string()))?;
        Ok(())
    }
}

#[async_trait::async_trait]
impl carbon_core::postgres::operations::Upsert for UpdateUserPerpPositionCustomMarginRatioRow {
    async fn upsert(&self, pool: &sqlx::PgPool) -> carbon_core::error::CarbonResult<()> {
        sqlx::query(
            r#"INSERT INTO update_user_perp_position_custom_margin_ratio_instruction (
                "sub_account_id",
                "perp_market_index",
                "margin_ratio",
                __signature, __instruction_index, __stack_height, __slot, __accounts
            ) VALUES (
                $1, $2, $3, $4, $5, $6, $7, $8
            ) ON CONFLICT (
                __signature, __instruction_index, __stack_height
            ) DO UPDATE SET
                "sub_account_id" = EXCLUDED."sub_account_id",
                "perp_market_index" = EXCLUDED."perp_market_index",
                "margin_ratio" = EXCLUDED."margin_ratio",
                __instruction_index = EXCLUDED.__instruction_index,
                __stack_height = EXCLUDED.__stack_height,
                __slot = EXCLUDED.__slot,
                __accounts = EXCLUDED.__accounts
            "#,
        )
        .bind(self.sub_account_id)
        .bind(self.perp_market_index)
        .bind(self.margin_ratio)
        .bind(&self.instruction_metadata.signature)
        .bind(self.instruction_metadata.instruction_index)
        .bind(self.instruction_metadata.stack_height)
        .bind(&self.instruction_metadata.slot)
        .bind(&self.accounts)
        .execute(pool)
        .await
        .map_err(|e| carbon_core::error::Error::Custom(e.to_string()))?;
        Ok(())
    }
}

#[async_trait::async_trait]
impl carbon_core::postgres::operations::Delete for UpdateUserPerpPositionCustomMarginRatioRow {
    type Key = (
        String,
        carbon_core::postgres::primitives::U32,
        carbon_core::postgres::primitives::U32,
    );

    async fn delete(key: Self::Key, pool: &sqlx::PgPool) -> carbon_core::error::CarbonResult<()> {
        sqlx::query(
            r#"DELETE FROM update_user_perp_position_custom_margin_ratio_instruction WHERE
                __signature = $1 AND __instruction_index = $2 AND __stack_height = $3
            "#,
        )
        .bind(key.0)
        .bind(key.1)
        .bind(key.2)
        .execute(pool)
        .await
        .map_err(|e| carbon_core::error::Error::Custom(e.to_string()))?;
        Ok(())
    }
}

#[async_trait::async_trait]
impl carbon_core::postgres::operations::LookUp for UpdateUserPerpPositionCustomMarginRatioRow {
    type Key = (
        String,
        carbon_core::postgres::primitives::U32,
        carbon_core::postgres::primitives::U32,
    );

    async fn lookup(
        key: Self::Key,
        pool: &sqlx::PgPool,
    ) -> carbon_core::error::CarbonResult<Option<Self>> {
        let row = sqlx::query_as(
            r#"SELECT * FROM update_user_perp_position_custom_margin_ratio_instruction WHERE
                __signature = $1 AND __instruction_index = $2 AND __stack_height = $3
            "#,
        )
        .bind(key.0)
        .bind(key.1)
        .bind(key.2)
        .fetch_optional(pool)
        .await
        .map_err(|e| carbon_core::error::Error::Custom(e.to_string()))?;
        Ok(row)
    }
}

pub struct UpdateUserPerpPositionCustomMarginRatioMigrationOperation;

#[async_trait::async_trait]
impl sqlx_migrator::Operation<sqlx::Postgres>
    for UpdateUserPerpPositionCustomMarginRatioMigrationOperation
{
    async fn up(
        &self,
        connection: &mut sqlx::PgConnection,
    ) -> Result<(), sqlx_migrator::error::Error> {
        sqlx::query(r#"CREATE TABLE IF NOT EXISTS update_user_perp_position_custom_margin_ratio_instruction (
                -- Instruction data
                "sub_account_id" INT4 NOT NULL,
                "perp_market_index" INT4 NOT NULL,
                "margin_ratio" INT4 NOT NULL,
                -- Instruction metadata
                __signature TEXT NOT NULL,
                __instruction_index BIGINT NOT NULL,
                __stack_height BIGINT NOT NULL,
                __slot NUMERIC(20),
                __accounts JSONB NOT NULL,
                PRIMARY KEY (__signature, __instruction_index, __stack_height)
            )"#).execute(connection).await?;
        Ok(())
    }

    async fn down(
        &self,
        connection: &mut sqlx::PgConnection,
    ) -> Result<(), sqlx_migrator::error::Error> {
        sqlx::query(
            r#"DROP TABLE IF EXISTS update_user_perp_position_custom_margin_ratio_instruction"#,
        )
        .execute(connection)
        .await?;
        Ok(())
    }
}
