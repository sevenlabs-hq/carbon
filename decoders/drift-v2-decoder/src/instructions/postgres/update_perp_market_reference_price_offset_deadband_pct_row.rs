...
impl UpdatePerpMarketReferencePriceOffsetDeadbandPctRow {
    pub fn from_parts(
        source: crate::instructions::update_perp_market_reference_price_offset_deadband_pct::UpdatePerpMarketReferencePriceOffsetDeadbandPct,
        metadata: InstructionMetadata,
        accounts: Vec<solana_instruction::AccountMeta>,
    ) -> Self {
        Self {
            instruction_metadata: metadata.into(),
            reference_price_offset_deadband_pct: source.reference_price_offset_deadband_pct.into(),
            accounts: sqlx::types::Json(accounts),
        }
    }
}

impl TryFrom<UpdatePerpMarketReferencePriceOffsetDeadbandPctRow> for crate::instructions::update_perp_market_reference_price_offset_deadband_pct::UpdatePerpMarketReferencePriceOffsetDeadbandPct {
    type Error = carbon_core::error::Error;
    fn try_from(source: UpdatePerpMarketReferencePriceOffsetDeadbandPctRow) -> Result<Self, Self::Error> {
        Ok(Self {
            reference_price_offset_deadband_pct: source.reference_price_offset_deadband_pct.try_into().map_err(|_| carbon_core::error::Error::Custom("Failed to convert value from postgres primitive".to_string()))?,
        })
    }
}
    ...
    for UpdatePerpMarketReferencePriceOffsetDeadbandPctRow
{
    async fn upsert(&self, pool: &sqlx::PgPool) -> carbon_core::error::CarbonResult<()> {
        sqlx::query(r#"INSERT INTO update_perp_market_reference_price_offset_deadband_pct_instruction (
                ...
                "reference_price_offset_deadband_pct" = EXCLUDED."reference_price_offset_deadband_pct",
...
pub struct UpdatePerpMarketReferencePriceOffsetDeadbandPctMigrationOperation;

#[async_trait::async_trait]
impl sqlx_migrator::Operation<sqlx::Postgres>
    for UpdatePerpMarketReferencePriceOffsetDeadbandPctMigrationOperation
{
    async fn up(
        &self,
