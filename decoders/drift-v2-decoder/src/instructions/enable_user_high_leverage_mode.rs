//! This code was AUTOGENERATED using the Codama library.
use carbon_core::{
    account_utils::next_account,
    deserialize::{ArrangeAccounts, CarbonDeserialize},
};
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Debug, Clone, borsh::BorshSerialize, borsh::BorshDeserialize, PartialEq)]
pub struct EnableUserHighLeverageMode {
    pub sub_account_id: u16,
}

#[derive(Debug, Clone, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct EnableUserHighLeverageModeInstructionAccounts {
    pub state: solana_pubkey::Pubkey,
    pub user: solana_pubkey::Pubkey,
    pub authority: solana_pubkey::Pubkey,
    pub high_leverage_mode_config: solana_pubkey::Pubkey,
    pub remaining: Vec<solana_instruction::AccountMeta>,
}

impl CarbonDeserialize for EnableUserHighLeverageMode {
    const DISCRIMINATOR: &'static [u8] = &[231, 24, 230, 112, 201, 173, 73, 184];

    fn decode(data: &[u8]) -> Option<Self> {
        if data.len() < 8 {
            return None;
        }
        let discriminator = &data[0..8];
        if discriminator != Self::DISCRIMINATOR {
            return None;
        }

        let mut data_slice = data;

        data_slice = &data_slice[8..];

        borsh::BorshDeserialize::deserialize(&mut data_slice).ok()
    }
}

impl ArrangeAccounts for EnableUserHighLeverageMode {
    type ArrangedAccounts = EnableUserHighLeverageModeInstructionAccounts;

    fn arrange_accounts(
        accounts: &[solana_instruction::AccountMeta],
    ) -> Option<Self::ArrangedAccounts> {
        let mut iter = accounts.iter();

        let state = next_account(&mut iter)?;
        let user = next_account(&mut iter)?;
        let authority = next_account(&mut iter)?;
        let high_leverage_mode_config = next_account(&mut iter)?;

        let remaining = iter.as_slice();

        Some(EnableUserHighLeverageModeInstructionAccounts {
            state,
            user,
            authority,
            high_leverage_mode_config,
            remaining: remaining.to_vec(),
        })
    }
}
