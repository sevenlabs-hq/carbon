//! This code was AUTOGENERATED using the Codama library.
use {
    crate::types::SpotFulfillmentConfigStatus,
    carbon_core::{
        account_utils::next_account,
        deserialize::{ArrangeAccounts, CarbonDeserialize},
    },
};
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Debug, Clone, borsh::BorshSerialize, borsh::BorshDeserialize, PartialEq)]
pub struct OpenbookV2FulfillmentConfigStatus {
    pub status: SpotFulfillmentConfigStatus,
}

#[derive(Debug, Clone, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct OpenbookV2FulfillmentConfigStatusInstructionAccounts {
    pub state: solana_pubkey::Pubkey,
    pub openbook_v2_fulfillment_config: solana_pubkey::Pubkey,
    pub admin: solana_pubkey::Pubkey,
    pub remaining: Vec<solana_instruction::AccountMeta>,
}

impl CarbonDeserialize for OpenbookV2FulfillmentConfigStatus {
    const DISCRIMINATOR: &'static [u8] = &[25, 173, 19, 189, 4, 211, 64, 238];

    fn decode(data: &[u8]) -> Option<Self> {
        if data.len() < 8 {
            return None;
        }
        let discriminator = &data[0..8];
        if discriminator != Self::DISCRIMINATOR {
            return None;
        }

        let mut data_slice = data;

        data_slice = &data_slice[8..];

        borsh::BorshDeserialize::deserialize(&mut data_slice).ok()
    }
}

impl ArrangeAccounts for OpenbookV2FulfillmentConfigStatus {
    type ArrangedAccounts = OpenbookV2FulfillmentConfigStatusInstructionAccounts;

    fn arrange_accounts(
        accounts: &[solana_instruction::AccountMeta],
    ) -> Option<Self::ArrangedAccounts> {
        let mut iter = accounts.iter();

        let state = next_account(&mut iter)?;
        let openbook_v2_fulfillment_config = next_account(&mut iter)?;
        let admin = next_account(&mut iter)?;

        let remaining = iter.as_slice();

        Some(OpenbookV2FulfillmentConfigStatusInstructionAccounts {
            state,
            openbook_v2_fulfillment_config,
            admin,
            remaining: remaining.to_vec(),
        })
    }
}
