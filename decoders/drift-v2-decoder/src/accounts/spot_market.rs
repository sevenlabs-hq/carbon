...
pub struct SpotMarket {
    ...
    /// The amount in the vault should be equal to or greater than deposits -
    /// borrows
    pub vault: Pubkey,
    ...
    /// e.g. for SOL-PERP, funds can be settled in usdc and will flow into the
    /// USDC revenue pool
    ...
    /// Details on the insurance fund covering bankruptcies in this markets
    /// token Covers bankruptcies for borrows with this markets token and
    /// perps settling in this markets token
    pub insurance_fund: InsuranceFund,
    /// The total spot fees collected for this market
    /// precision: QUOTE_PRECISION
    pub total_spot_fee: u128,
    /// The sum of the scaled balances for deposits across users and pool
    /// balances To convert to the deposit token amount, multiply by the
    /// cumulative deposit interest precision: SPOT_BALANCE_PRECISION
    pub deposit_balance: u128,
    /// The sum of the scaled balances for borrows across users and pool
    /// balances To convert to the borrow token amount, multiply by the
    /// cumulative borrow interest precision: SPOT_BALANCE_PRECISION
    ...
    /// The time the market is set to expire. Only set if market is in reduce
    /// only mode
    pub expiry_ts: i64,
    ...
    /// The initial asset weight used to calculate a deposits contribution to a
    /// users initial total collateral e.g. if the asset weight is .8, $100
    /// of deposits contributes $80 to the users initial total collateral
    /// precision: SPOT_WEIGHT_PRECISION
    pub initial_asset_weight: u32,
    /// The maintenance asset weight used to calculate a deposits contribution
    /// to a users maintenance total collateral e.g. if the asset weight is
    /// .9, $100 of deposits contributes $90 to the users maintenance total
    /// collateral precision: SPOT_WEIGHT_PRECISION
    pub maintenance_asset_weight: u32,
    /// The initial liability weight used to calculate a borrows contribution to
    /// a users initial margin requirement e.g. if the liability weight is
    /// .9, $100 of borrows contributes $90 to the users initial margin
    /// requirement precision: SPOT_WEIGHT_PRECISION
    pub initial_liability_weight: u32,
    /// The maintenance liability weight used to calculate a borrows
    /// contribution to a users maintenance margin requirement e.g. if the
    /// liability weight is .8, $100 of borrows contributes $80 to the users
    /// maintenance margin requirement precision: SPOT_WEIGHT_PRECISION
    pub maintenance_liability_weight: u32,
    /// The initial margin fraction factor. Used to increase liability
    /// weight/decrease asset weight for large positions precision:
    /// MARGIN_PRECISION
    pub imf_factor: u32,
    ...
    /// The market's token mint's decimals. To from decimals to a precision,
    /// 10^decimals
    pub decimals: u32,
    ...
    /// The asset tier affects how a deposit can be used as collateral and the
    /// priority for a borrow being liquidated
    ...
    /// The min borrow rate for this market when the market regardless of
    /// utilization 1 => 1/200 => .5%
    /// precision: X/200
