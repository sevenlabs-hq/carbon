...
use {
    crate::types::{HistoricalOracleData, OracleSource, PoolBalance},
...
pub struct AMM {
    ...
    /// drift caused by integer math issues precision: AMM_RESERVE_PRECISION
    pub sqrt_k: u128,
    /// normalizing numerical factor for y, its use offers lowest slippage in
    /// cp-curve when market is balanced precision: PEG_PRECISION
    pub peg_multiplier: u128,
    /// y when market is balanced. stored to save computation
    /// precision: AMM_RESERVE_PRECISION
    pub terminal_quote_asset_reserve: u128,
    /// always non-negative. tracks number of total longs in market (regardless
    /// of counterparty) precision: BASE_PRECISION
    pub base_asset_amount_long: i128,
    /// always non-positive. tracks number of total shorts in market (regardless
    /// of counterparty) precision: BASE_PRECISION
    ...
    /// total user lp shares of sqrt_k (protocol owned liquidity = sqrt_k -
    /// last_funding_rate) precision: AMM_RESERVE_PRECISION
    ...
    /// last funding rate for shorts in this perp market (unit is quote per
    /// base) precision: QUOTE_PRECISION
    pub last_funding_rate_short: i64,
    /// estimate of last 24h of funding rate perp market (unit is quote per
    /// base) precision: QUOTE_PRECISION
    ...
    /// the gap between the oracle price and the reserve price = y *
    /// peg_multiplier / x
    ...
    /// the minimum spread the AMM can quote. also used as step size for some
    /// spread logic increases.
    pub base_spread: u32,
    ...
    /// the fraction of total available liquidity a single fill on the AMM can
    /// consume
    ...
    /// the jit intensity of AMM. larger intensity means larger participation in
    /// jit. 0 means no jit participation. (0, 100] is intensity for
    /// protocol-owned AMM. (100, 200] is intensity for user LP-owned AMM.
    pub amm_jit_intensity: u8,
    /// the oracle provider information. used to decode/scale the oracle public
    /// key
    pub oracle_source: OracleSource,
    /// tracks whether the oracle was considered valid at the last AMM update
    pub last_oracle_valid: bool,
    /// the target value for `base_asset_amount_per_lp`, used during AMM JIT
    /// with LP split precision: BASE_PRECISION
    pub target_base_asset_amount_per_lp: i32,
    /// expo for unit of per_lp, base 10 (if per_lp_base=X, then per_lp unit is
    /// 10^X)
    pub per_lp_base: i8,
    ...
    /// signed scale amm_spread similar to fee_adjustment logic (-100 = 0, 100 =
    /// double)
