...
impl carbon_core::clickhouse::ClusterTable for TransferOwnershipRow {
    fn local_table() -> &'static str {
        "circle_message_transmitter_v2_transfer_ownership_instruction_local"
    }
...
impl carbon_core::clickhouse::Operation for TransferOwnershipRowMigrationOperation {
    async fn up(
        ...
        let replica_path = format!(
            "/clickhouse/tables/{}/circle_message_transmitter_v2_transfer_ownership_instruction_local",
            config.shard_name,
        );
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS circle_message_transmitter_v2_transfer_ownership_instruction_local ON CLUSTER {cluster} (
                        __signature String,
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS circle_message_transmitter_v2_transfer_ownership_instruction ON CLUSTER {cluster} AS circle_message_transmitter_v2_transfer_ownership_instruction_local
                    ENGINE = Distributed({cluster}, currentDatabase(), circle_message_transmitter_v2_transfer_ownership_instruction_local, rand())
                "#,
                cluster = cluster_name,
            ))
            .execute()
    ...
    async fn down(
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS circle_message_transmitter_v2_transfer_ownership_instruction ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS circle_message_transmitter_v2_transfer_ownership_instruction_local ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
...
impl carbon_core::clickhouse::Operation for TransferOwnershipRowMigrationOperation {
    async fn up(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query(
                r#"
                    CREATE TABLE IF NOT EXISTS circle_message_transmitter_v2_transfer_ownership_instruction (
                        __signature String,
                        __instruction_index UInt32,
                        __stack_height UInt32,
                        __slot UInt64,
