...
impl carbon_core::clickhouse::ClusterTable for BurnEditionNftRow {
    fn local_table() -> &'static str {
        "mpl_token_metadata_burn_edition_nft_instruction_local"
    }
...
impl carbon_core::clickhouse::Table for BurnEditionNftRow {
    fn table() -> &'static str {
        "mpl_token_metadata_burn_edition_nft_instruction"
    }
}
...
pub struct BurnEditionNftRowMigrationOperation;

#[cfg(feature = "clickhouse-cluster")]
#[async_trait::async_trait]
impl carbon_core::clickhouse::Operation for BurnEditionNftRowMigrationOperation {
    async fn up(
        &self,
        client: &clickhouse::Client,
        ...
        let replica_path = format!(
            "/clickhouse/tables/{}/mpl_token_metadata_burn_edition_nft_instruction_local",
            config.shard_name,
        );
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS mpl_token_metadata_burn_edition_nft_instruction_local ON CLUSTER {cluster} (
                        __signature String,
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS mpl_token_metadata_burn_edition_nft_instruction ON CLUSTER {cluster} AS mpl_token_metadata_burn_edition_nft_instruction_local
                    ENGINE = Distributed({cluster}, currentDatabase(), mpl_token_metadata_burn_edition_nft_instruction_local, rand())
                "#,
                cluster = cluster_name,
            ))
            .execute()
    ...
    async fn down(
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS mpl_token_metadata_burn_edition_nft_instruction ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS mpl_token_metadata_burn_edition_nft_instruction_local ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
...
impl carbon_core::clickhouse::Operation for BurnEditionNftRowMigrationOperation {
    async fn up(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query(
                r#"
                    CREATE TABLE IF NOT EXISTS mpl_token_metadata_burn_edition_nft_instruction (
                        __signature String,
                        __instruction_index UInt32,
                        __stack_height UInt32,
                        __slot UInt64,
    ...
    async fn down(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query("DROP TABLE IF EXISTS mpl_token_metadata_burn_edition_nft_instruction")
            .execute()
            .await?;
