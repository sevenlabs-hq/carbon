//! This code was AUTOGENERATED using the Codama library.
use carbon_core::instruction::InstructionMetadata;
use carbon_core::postgres::metadata::InstructionRowMetadata;
use carbon_core::postgres::primitives::Pubkey;
use carbon_core::postgres::primitives::U64;
use carbon_core::postgres::primitives::U8;

#[derive(sqlx::FromRow, Debug, Clone)]
pub struct InitializeFeeProgramGlobalRow {
    #[sqlx(flatten)]
    pub instruction_metadata: InstructionRowMetadata,
    pub social_claim_authority: Pubkey,
    pub disable_flags: U8,
    pub claim_rate_limit: U64,
}

impl InitializeFeeProgramGlobalRow {
    pub fn from_parts(source: crate::instructions::initialize_fee_program_global::InitializeFeeProgramGlobal, metadata: InstructionMetadata) -> Self {
        Self {
            instruction_metadata: metadata.into(),
            social_claim_authority: source.social_claim_authority.into(),
            disable_flags: source.disable_flags.into(),
            claim_rate_limit: source.claim_rate_limit.into(),
        }
    }
}

impl TryFrom<InitializeFeeProgramGlobalRow> for crate::instructions::initialize_fee_program_global::InitializeFeeProgramGlobal {
    type Error = carbon_core::error::Error;
    fn try_from(source: InitializeFeeProgramGlobalRow) -> Result<Self, Self::Error> {
        Ok(Self {
            social_claim_authority: *source.social_claim_authority,
            disable_flags: source.disable_flags.try_into().map_err(|_| carbon_core::error::Error::Custom("Failed to convert value from postgres primitive".to_string()))?,
            claim_rate_limit: *source.claim_rate_limit,
        })
    }
}

impl carbon_core::postgres::operations::Table for crate::instructions::initialize_fee_program_global::InitializeFeeProgramGlobal {
    fn table() -> &'static str {
        "initialize_fee_program_global_instruction"
    }

    fn columns() -> Vec<&'static str> {
        vec![
            "__signature",
            "__instruction_index",
            "__stack_height",
            "__slot",
            "social_claim_authority",
            "disable_flags",
            "claim_rate_limit",
        ]
    }
}

#[async_trait::async_trait]
impl carbon_core::postgres::operations::Insert for InitializeFeeProgramGlobalRow {
    async fn insert(&self, pool: &sqlx::PgPool) -> carbon_core::error::CarbonResult<()> {
        sqlx::query(r#"
            INSERT INTO initialize_fee_program_global_instruction (
                "social_claim_authority",
                "disable_flags",
                "claim_rate_limit",
                __signature, __instruction_index, __stack_height, __slot
            ) VALUES (
                $1, $2, $3, $4, $5, $6, $7
            )"#)
        .bind(self.social_claim_authority.clone())
        .bind(self.disable_flags.clone())
        .bind(self.claim_rate_limit.clone())
        .bind(self.instruction_metadata.signature.clone())
        .bind(self.instruction_metadata.instruction_index.clone())
        .bind(self.instruction_metadata.stack_height.clone())
        .bind(self.instruction_metadata.slot.clone())
        .execute(pool).await
        .map_err(|e| carbon_core::error::Error::Custom(e.to_string()))?;
        Ok(())
    }
}

#[async_trait::async_trait]
impl carbon_core::postgres::operations::Upsert for InitializeFeeProgramGlobalRow {
    async fn upsert(&self, pool: &sqlx::PgPool) -> carbon_core::error::CarbonResult<()> {
        sqlx::query(r#"INSERT INTO initialize_fee_program_global_instruction (
                "social_claim_authority",
                "disable_flags",
                "claim_rate_limit",
                __signature, __instruction_index, __stack_height, __slot
            ) VALUES (
                $1, $2, $3, $4, $5, $6, $7
            ) ON CONFLICT (
                __signature, __instruction_index, __stack_height
            ) DO UPDATE SET
                "social_claim_authority" = EXCLUDED."social_claim_authority",
                "disable_flags" = EXCLUDED."disable_flags",
                "claim_rate_limit" = EXCLUDED."claim_rate_limit",
                __instruction_index = EXCLUDED.__instruction_index,
                __stack_height = EXCLUDED.__stack_height,
                __slot = EXCLUDED.__slot
            "#)
        .bind(self.social_claim_authority.clone())
        .bind(self.disable_flags.clone())
        .bind(self.claim_rate_limit.clone())
        .bind(self.instruction_metadata.signature.clone())
        .bind(self.instruction_metadata.instruction_index.clone())
        .bind(self.instruction_metadata.stack_height.clone())
        .bind(self.instruction_metadata.slot.clone())
        .execute(pool).await
        .map_err(|e| carbon_core::error::Error::Custom(e.to_string()))?;
        Ok(())
    }
}

#[async_trait::async_trait]
impl carbon_core::postgres::operations::Delete for InitializeFeeProgramGlobalRow {
    type Key = (String, carbon_core::postgres::primitives::U32, carbon_core::postgres::primitives::U32);

    async fn delete(key: Self::Key, pool: &sqlx::PgPool) -> carbon_core::error::CarbonResult<()> {
        sqlx::query(r#"DELETE FROM initialize_fee_program_global_instruction WHERE
                __signature = $1 AND __instruction_index = $2 AND __stack_height = $3
            "#)
        .bind(key.0)
        .bind(key.1)
        .bind(key.2)
        .execute(pool).await
        .map_err(|e| carbon_core::error::Error::Custom(e.to_string()))?;
        Ok(())
    }
}

#[async_trait::async_trait]
impl carbon_core::postgres::operations::LookUp for InitializeFeeProgramGlobalRow {
    type Key = (String, carbon_core::postgres::primitives::U32, carbon_core::postgres::primitives::U32);

    async fn lookup(key: Self::Key, pool: &sqlx::PgPool) -> carbon_core::error::CarbonResult<Option<Self>> {
        let row = sqlx::query_as(r#"SELECT * FROM initialize_fee_program_global_instruction WHERE
                __signature = $1 AND __instruction_index = $2 AND __stack_height = $3
            "#)
        .bind(key.0)
        .bind(key.1)
        .bind(key.2)
        .fetch_optional(pool).await
        .map_err(|e| carbon_core::error::Error::Custom(e.to_string()))?;
        Ok(row)
    }
}

pub struct InitializeFeeProgramGlobalMigrationOperation;

#[async_trait::async_trait]
impl sqlx_migrator::Operation<sqlx::Postgres> for InitializeFeeProgramGlobalMigrationOperation {
    async fn up(&self, connection: &mut sqlx::PgConnection) -> Result<(), sqlx_migrator::error::Error> {
        sqlx::query(r#"CREATE TABLE IF NOT EXISTS initialize_fee_program_global_instruction (
                -- Instruction data
                "social_claim_authority" BYTEA NOT NULL,
                "disable_flags" INT2 NOT NULL,
                "claim_rate_limit" NUMERIC(20) NOT NULL,
                -- Instruction metadata
                __signature TEXT NOT NULL,
                __instruction_index BIGINT NOT NULL,
                __stack_height BIGINT NOT NULL,
                __slot NUMERIC(20),
                PRIMARY KEY (__signature, __instruction_index, __stack_height)
            )"#).execute(connection).await?;
        Ok(())
    }

    async fn down(&self, connection: &mut sqlx::PgConnection) -> Result<(), sqlx_migrator::error::Error> {
        sqlx::query(r#"DROP TABLE IF EXISTS initialize_fee_program_global_instruction"#).execute(connection).await?;
        Ok(())
    }
}
