...
pub use self::buyback_vault_row::*;
...
pub struct PumpFeesAccountWithMetadata(pub PumpFeesAccount, pub carbon_core::account::AccountMetadata);

impl From<(PumpFeesAccount, carbon_core::account::AccountMetadata)> for PumpFeesAccountWithMetadata {
    ...
    async fn insert(&self, pool: &sqlx::PgPool) -> carbon_core::error::CarbonResult<()> {
        ...
        match account {
            ...
            PumpFeesAccount::BuybackVault(account) => {
                let row = buyback_vault_row::BuybackVaultRow::from_parts(*account.clone(), metadata.clone());
                row.insert(pool).await?;
                Ok(())
            }
            ...
            PumpFeesAccount::FeeProgramGlobal(account) => {
                let row = fee_program_global_row::FeeProgramGlobalRow::from_parts(*account.clone(), metadata.clone());
                row.insert(pool).await?;
                Ok(())
            }
            ...
            PumpFeesAccount::SocialFeePda(account) => {
                let row = social_fee_pda_row::SocialFeePdaRow::from_parts(*account.clone(), metadata.clone());
                row.insert(pool).await?;
                Ok(())
            }
        }
