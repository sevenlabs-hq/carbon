//! This code was AUTOGENERATED using the Codama library.
use {
    crate::types::KaminoConfigCompact,
    carbon_core::{
        account_utils::next_account,
        deserialize::{ArrangeAccounts, CarbonDeserialize},
    },
};
/// (group admin only) Add a Kamino bank to the group. Pass the oracle and
/// reserve in remaining accounts 0 and 1 respectively.
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Debug, Clone, borsh::BorshSerialize, borsh::BorshDeserialize, PartialEq)]
pub struct LendingPoolAddBankKamino {
    pub bank_config: KaminoConfigCompact,
    pub bank_seed: u64,
}

#[derive(Debug, Clone, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct LendingPoolAddBankKaminoInstructionAccounts {
    pub group: solana_pubkey::Pubkey,
    pub admin: solana_pubkey::Pubkey,
    pub fee_payer: solana_pubkey::Pubkey,
    pub bank_mint: solana_pubkey::Pubkey,
    pub bank: solana_pubkey::Pubkey,
    pub integration_acc1: solana_pubkey::Pubkey,
    pub integration_acc2: solana_pubkey::Pubkey,
    pub liquidity_vault_authority: solana_pubkey::Pubkey,
    pub liquidity_vault: solana_pubkey::Pubkey,
    pub insurance_vault_authority: solana_pubkey::Pubkey,
    pub insurance_vault: solana_pubkey::Pubkey,
    pub fee_vault_authority: solana_pubkey::Pubkey,
    pub fee_vault: solana_pubkey::Pubkey,
    pub token_program: solana_pubkey::Pubkey,
    pub system_program: solana_pubkey::Pubkey,
    pub remaining: Vec<solana_instruction::AccountMeta>,
}

impl CarbonDeserialize for LendingPoolAddBankKamino {
    const DISCRIMINATOR: &'static [u8] = &[118, 53, 16, 243, 255, 245, 149, 241];

    fn decode(data: &[u8]) -> Option<Self> {
        if data.len() < 8 {
            return None;
        }
        let discriminator = &data[0..8];
        if discriminator != Self::DISCRIMINATOR {
            return None;
        }

        let mut data_slice = data;

        data_slice = &data_slice[8..];

        borsh::BorshDeserialize::deserialize(&mut data_slice).ok()
    }
}

impl ArrangeAccounts for LendingPoolAddBankKamino {
    type ArrangedAccounts = LendingPoolAddBankKaminoInstructionAccounts;

    fn arrange_accounts(
        accounts: &[solana_instruction::AccountMeta],
    ) -> Option<Self::ArrangedAccounts> {
        let mut iter = accounts.iter();

        let group = next_account(&mut iter)?;
        let admin = next_account(&mut iter)?;
        let fee_payer = next_account(&mut iter)?;
        let bank_mint = next_account(&mut iter)?;
        let bank = next_account(&mut iter)?;
        let integration_acc1 = next_account(&mut iter)?;
        let integration_acc2 = next_account(&mut iter)?;
        let liquidity_vault_authority = next_account(&mut iter)?;
        let liquidity_vault = next_account(&mut iter)?;
        let insurance_vault_authority = next_account(&mut iter)?;
        let insurance_vault = next_account(&mut iter)?;
        let fee_vault_authority = next_account(&mut iter)?;
        let fee_vault = next_account(&mut iter)?;
        let token_program = next_account(&mut iter)?;
        let system_program = next_account(&mut iter)?;

        let remaining = iter.as_slice();

        Some(LendingPoolAddBankKaminoInstructionAccounts {
            group,
            admin,
            fee_payer,
            bank_mint,
            bank,
            integration_acc1,
            integration_acc2,
            liquidity_vault_authority,
            liquidity_vault,
            insurance_vault_authority,
            insurance_vault,
            fee_vault_authority,
            fee_vault,
            token_program,
            system_program,
            remaining: remaining.to_vec(),
        })
    }
}
