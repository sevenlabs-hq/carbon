...
impl carbon_core::clickhouse::ClusterTable for MintV1Row {
    fn local_table() -> &'static str {
        "bubblegum_mint_v1_instruction_local"
    }
...
impl carbon_core::clickhouse::Table for MintV1Row {
    fn table() -> &'static str {
        "bubblegum_mint_v1_instruction"
    }
}
...
pub struct MintV1RowMigrationOperation;

#[cfg(feature = "clickhouse-cluster")]
#[async_trait::async_trait]
impl carbon_core::clickhouse::Operation for MintV1RowMigrationOperation {
    async fn up(
        &self,
        client: &clickhouse::Client,
        ...
        let replica_path = format!(
            "/clickhouse/tables/{}/bubblegum_mint_v1_instruction_local",
            config.shard_name,
        );
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS bubblegum_mint_v1_instruction_local ON CLUSTER {cluster} (
                        __signature String,
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS bubblegum_mint_v1_instruction ON CLUSTER {cluster} AS bubblegum_mint_v1_instruction_local
                    ENGINE = Distributed({cluster}, currentDatabase(), bubblegum_mint_v1_instruction_local, rand())
                "#,
                cluster = cluster_name,
            ))
            .execute()
    ...
    async fn down(
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS bubblegum_mint_v1_instruction ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS bubblegum_mint_v1_instruction_local ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
...
impl carbon_core::clickhouse::Operation for MintV1RowMigrationOperation {
    async fn up(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query(
                r#"
                    CREATE TABLE IF NOT EXISTS bubblegum_mint_v1_instruction (
                        __signature String,
                        __instruction_index UInt32,
                        __stack_height UInt32,
                        __slot UInt64,
    ...
    async fn down(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query("DROP TABLE IF EXISTS bubblegum_mint_v1_instruction")
            .execute()
            .await?;
