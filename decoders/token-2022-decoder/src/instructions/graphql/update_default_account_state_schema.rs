//! This code was AUTOGENERATED using the Codama library.
//!
//! <https://github.com/codama-idl/codama>
//!
use carbon_core::graphql::primitives::U8;
use crate::types::graphql::AccountStateGraphQL;
use juniper::GraphQLObject;

#[derive(Debug, Clone, GraphQLObject)]
#[graphql(name = "UpdateDefaultAccountState")]
pub struct UpdateDefaultAccountStateGraphQL {
    pub instruction_metadata: crate::instructions::graphql::InstructionMetadataGraphQL,
    pub default_account_state_discriminator: U8,
    pub state: AccountStateGraphQL,
}

impl TryFrom<crate::instructions::postgres::UpdateDefaultAccountStateRow> for UpdateDefaultAccountStateGraphQL {
    type Error = carbon_core::error::Error;
    fn try_from(row: crate::instructions::postgres::UpdateDefaultAccountStateRow) -> Result<Self, Self::Error> {
        Ok(Self {
            instruction_metadata: row.instruction_metadata.into(),
            default_account_state_discriminator: carbon_core::graphql::primitives::U8((*row.default_account_state_discriminator) as u8),
            state: row.state.0.into(),
        })
    }
}
