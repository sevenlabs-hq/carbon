...
impl carbon_core::clickhouse::ClusterTable for ThawAccountRow {
    fn local_table() -> &'static str {
        "token_2022_thaw_account_instruction_local"
    }

    fn distributed_table() -> &'static str {
        "token_2022_thaw_account_instruction"
    }
}

#[cfg(not(feature = "clickhouse-cluster"))]
impl carbon_core::clickhouse::Table for ThawAccountRow {
    fn table() -> &'static str {
        "token_2022_thaw_account_instruction"
    }
}
...
impl carbon_core::clickhouse::Insert for ThawAccountRow {
    async fn insert(
        ...
        let mut insert = client
            .insert::<Self>("token_2022_thaw_account_instruction")
            .await
            .map_err(|error| carbon_core::error::Error::Custom(error.to_string()))?;
...
pub struct ThawAccountRowMigrationOperation;

#[cfg(feature = "clickhouse-cluster")]
#[async_trait::async_trait]
impl carbon_core::clickhouse::Operation for ThawAccountRowMigrationOperation {
    async fn up(
        &self,
        client: &clickhouse::Client,
        ...
        let replica_path = format!(
            "/clickhouse/tables/{}/token_2022_thaw_account_instruction_local",
            config.shard_name,
        );
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS token_2022_thaw_account_instruction_local ON CLUSTER {cluster} (
                        __signature String,
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS token_2022_thaw_account_instruction ON CLUSTER {cluster} AS token_2022_thaw_account_instruction_local
                    ENGINE = Distributed({cluster}, currentDatabase(), token_2022_thaw_account_instruction_local, rand())
                "#,
                cluster = cluster_name,
            ))
            .execute()
    ...
    async fn down(
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS token_2022_thaw_account_instruction ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS token_2022_thaw_account_instruction_local ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
...
impl carbon_core::clickhouse::Operation for ThawAccountRowMigrationOperation {
    async fn up(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query(
                r#"
                    CREATE TABLE IF NOT EXISTS token_2022_thaw_account_instruction (
                        __signature String,
                        __instruction_index UInt32,
                        __stack_height UInt32,
                        __slot UInt64,
    ...
    async fn down(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query("DROP TABLE IF EXISTS token_2022_thaw_account_instruction")
            .execute()
            .await?;
