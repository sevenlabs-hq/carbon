...
pub enum CpiEvent {
    ...
    JitHopEvent(events::jit_hop_event::JitHopEventEvent),
    ...
    SwapCompleteEvent(events::swap_complete_event::SwapCompleteEventEvent),
...
impl CpiEvent {
    pub fn decode(data: &[u8]) -> Option<Self> {
        ...
        if let Some(decoded) = events::jit_hop_event::JitHopEventEvent::decode(event_data) {
            return Some(CpiEvent::JitHopEvent(decoded));
        }
        if let Some(decoded) =
        ...
        if let Some(decoded) =
            events::swap_complete_event::SwapCompleteEventEvent::decode(event_data)
        {
            return Some(CpiEvent::SwapCompleteEvent(decoded));
        }
