...
impl carbon_core::clickhouse::ClusterTable for PermissionRefundRow {
    fn local_table() -> &'static str {
        "wavebreak_permission_refund_instruction_local"
    }

    fn distributed_table() -> &'static str {
        "wavebreak_permission_refund_instruction"
    }
}

#[cfg(not(feature = "clickhouse-cluster"))]
impl carbon_core::clickhouse::Table for PermissionRefundRow {
    fn table() -> &'static str {
        "wavebreak_permission_refund_instruction"
    }
}
...
pub struct PermissionRefundRowMigrationOperation;

#[cfg(feature = "clickhouse-cluster")]
#[async_trait::async_trait]
impl carbon_core::clickhouse::Operation for PermissionRefundRowMigrationOperation {
    async fn up(
        &self,
        client: &clickhouse::Client,
        ...
        let replica_path = format!(
            "/clickhouse/tables/{}/wavebreak_permission_refund_instruction_local",
            config.shard_name,
        );
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS wavebreak_permission_refund_instruction_local ON CLUSTER {cluster} (
                        __signature String,
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS wavebreak_permission_refund_instruction ON CLUSTER {cluster} AS wavebreak_permission_refund_instruction_local
                    ENGINE = Distributed({cluster}, currentDatabase(), wavebreak_permission_refund_instruction_local, rand())
                "#,
                cluster = cluster_name,
            ))
            .execute()
    ...
    async fn down(
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS wavebreak_permission_refund_instruction ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS wavebreak_permission_refund_instruction_local ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
...
impl carbon_core::clickhouse::Operation for PermissionRefundRowMigrationOperation {
    async fn up(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query(
                r#"
                    CREATE TABLE IF NOT EXISTS wavebreak_permission_refund_instruction (
                        __signature String,
                        __instruction_index UInt32,
                        __stack_height UInt32,
                        __slot UInt64,
    ...
    async fn down(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query("DROP TABLE IF EXISTS wavebreak_permission_refund_instruction")
            .execute()
            .await?;
