...
impl carbon_core::clickhouse::ClusterTable for PreInitializeRow {
    fn local_table() -> &'static str {
        "raydium_amm_v4_pre_initialize_instruction_local"
    }

    fn distributed_table() -> &'static str {
        "raydium_amm_v4_pre_initialize_instruction"
    }
}
...
impl carbon_core::clickhouse::Insert for PreInitializeRow {
    async fn insert(
        ...
        let mut insert = client
            .insert::<Self>("raydium_amm_v4_pre_initialize_instruction")
            .await
            .map_err(|error| carbon_core::error::Error::Custom(error.to_string()))?;
...
impl carbon_core::clickhouse::Operation for PreInitializeRowMigrationOperation {
    async fn up(
        ...
        let replica_path = format!(
            "/clickhouse/tables/{}/raydium_amm_v4_pre_initialize_instruction_local",
            config.shard_name,
        );
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS raydium_amm_v4_pre_initialize_instruction_local ON CLUSTER {cluster} (
                        __signature String,
        ...
        client
            .query(&format!(
                r#"
                    CREATE TABLE IF NOT EXISTS raydium_amm_v4_pre_initialize_instruction ON CLUSTER {cluster} AS raydium_amm_v4_pre_initialize_instruction_local
                    ENGINE = Distributed({cluster}, currentDatabase(), raydium_amm_v4_pre_initialize_instruction_local, rand())
                "#,
                cluster = cluster_name,
            ))
            .execute()
    ...
    async fn down(
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS raydium_amm_v4_pre_initialize_instruction ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
        ...
        client
            .query(&format!(
                "DROP TABLE IF EXISTS raydium_amm_v4_pre_initialize_instruction_local ON CLUSTER {cluster}",
                cluster = cluster_name,
            ))
            .execute()
            .await?;
...
impl carbon_core::clickhouse::Operation for PreInitializeRowMigrationOperation {
    async fn up(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query(
                r#"
                    CREATE TABLE IF NOT EXISTS raydium_amm_v4_pre_initialize_instruction (
                        __signature String,
                        __instruction_index UInt32,
                        __stack_height UInt32,
                        __slot UInt64,
    ...
    async fn down(&self, client: &clickhouse::Client) -> clickhouse::error::Result<()> {
        client
            .query("DROP TABLE IF EXISTS raydium_amm_v4_pre_initialize_instruction")
            .execute()
            .await?;
