//! This code was AUTOGENERATED using the Codama library.
use carbon_core::{
    account_utils::next_account,
    deserialize::{ArrangeAccounts, CarbonDeserialize},
};
/// Permissionless instruction to check the minimum required fees for
/// distribution Returns the minimum required balance from the creator_vault and
/// whether distribution can proceed
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
#[derive(Debug, Clone, borsh::BorshSerialize, borsh::BorshDeserialize, PartialEq)]
pub struct GetMinimumDistributableFee {}

#[derive(Debug, Clone, PartialEq)]
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
pub struct GetMinimumDistributableFeeInstructionAccounts {
    pub mint: solana_pubkey::Pubkey,
    pub bonding_curve: solana_pubkey::Pubkey,
    pub sharing_config: solana_pubkey::Pubkey,
    pub creator_vault: solana_pubkey::Pubkey,
    pub remaining: Vec<solana_instruction::AccountMeta>,
}

impl CarbonDeserialize for GetMinimumDistributableFee {
    const DISCRIMINATOR: &'static [u8] = &[117, 225, 127, 202, 134, 95, 68, 35];

    fn decode(data: &[u8]) -> Option<Self> {
        if data.len() < 8 {
            return None;
        }
        let discriminator = &data[0..8];
        if discriminator != Self::DISCRIMINATOR {
            return None;
        }

        let mut data_slice = data;

        data_slice = &data_slice[8..];

        borsh::BorshDeserialize::deserialize(&mut data_slice).ok()
    }
}

impl ArrangeAccounts for GetMinimumDistributableFee {
    type ArrangedAccounts = GetMinimumDistributableFeeInstructionAccounts;

    fn arrange_accounts(
        accounts: &[solana_instruction::AccountMeta],
    ) -> Option<Self::ArrangedAccounts> {
        let mut iter = accounts.iter();

        let mint = next_account(&mut iter)?;
        let bonding_curve = next_account(&mut iter)?;
        let sharing_config = next_account(&mut iter)?;
        let creator_vault = next_account(&mut iter)?;

        let remaining = iter.as_slice();

        Some(GetMinimumDistributableFeeInstructionAccounts {
            mint,
            bonding_curve,
            sharing_config,
            creator_vault,
            remaining: remaining.to_vec(),
        })
    }
}
